...
### DuckDB parity restoration status

The DuckDB analytical port is the accepted public backend. The restoration work
reconciled route populations, labels, metrics, filters, payload shapes, snapshot
publication, and ordinary/component confidence intervals against the frozen
contract and independent same-source evidence. The production mixed-load gate,
transactional refresh, immutable deployment, readiness, and live gateway smoke
checks all pass. The review and completed repair sequence remain documented in
[`analysis/duckdb-migration-review-2026-09-22.md`](analysis/duckdb-migration-review-2026-09-22.md).

The machine-readable restoration contract is
`C:/Users/ascri/Desktop/ark-nova-function/contracts/dashboard-routes.v1.json`.
It covers all 50 route contracts and records the approved post-baseline changes
that must survive restoration. Its pinned baseline evidence, independent
synthetic oracle, and compact same-source real-data sample live beside it and
under the backend `tests/fixtures/` directory. Run the green foundation suite
with `python -m unittest tests.test_migration_contract_foundation`. Run the
strict current-artifact gate with `python phase5_contract_regression_gate.py`.

The canonical local population boundary is defined in
`C:/Users/ascri/Desktop/ark-nova-function/contracts/increment2-population-contract.md`.
Raw imports remain immutable; analytical routes use the prepared player-game
view with canonical opponent pre-match Elo, normalized starting position,
merged player identity, table-wide corruption/completion, Arena/tournament
membership, and strict MW telemetry. Refresh-owned event relations preserve
card plays, opening hands, endgames, sponsor endgames, project rewards, action
history, and MW action-card selections. Run its green gate with
`python phase2_population_contract_gate.py`.

Cards uses event-grain plays and excludes the 13 summary-project pseudo-cards;
Opening Hand uses dealt/kept opening events; Players uses its own Turns-first
catalog, merged identities, matrix payloads, per-map ordinary CIs, and rolling
histories. The standalone graph-only Elo history remains combined-dataset and
all-status while ignoring sidebar scope. Cards and Opening Hand abort or ignore
stale filter requests, including when a cached default is restored. The focused
fixture gate is `python increment3_view_contract_gate.py`; the strict release
gate is `python phase5_contract_regression_gate.py`. Historical Phase 4
count/schema audits are supporting evidence, not release authority.

Increment 4's private implementation restores all five Combos contracts and
all four MW Action Cards contracts. Combos now uses dataset-specific canonical
player-game populations, exact view-specific components, strict telemetry for
Card + Action Card, and a response projection for Minimum/search/Type/sort/rank/
pagination controls. MW Action Cards uses the authoritative twenty-card catalog,
strict two-player telemetry eligibility, a shared General/Draft payload,
same-map By map observations, and one selected pair per player-game for
Synergies. Visible standalone components use versioned table-clustered CI
batches and default snapshots are enriched with those same-version intervals;
additive Synergy intervals remain absent. The current contract is documented in
[`analysis/increment4-combos-mw-restoration.md`](analysis/increment4-combos-mw-restoration.md).
Run `python increment4_combo_mw_gate.py` from the backend working copy to
rebuild a real-schema sample and verify direct same-source aggregates, the
twenty-card catalog, component-CI batches, response projection, and frontend
syntax. Production acceptance additionally requires the current-generation
independent reconciliation and exact artifact-manifest gates described below.

The remaining analytical and external-source routes use their restored current
catalogs and denominators: Predictors, Actions, Build, Conservation, Scoring,
Icons, Workers, Maps, Endgames, Home, Arena, and Records. Home alone retains
corrupted games. Arena and Records moving sources use validated fresh-or-last-
known-good inputs, and tournament metadata falls back to its validated cache or
the active generation. The current backend contract is documented in
[`analysis/increment5-remaining-route-contract.md`](analysis/increment5-remaining-route-contract.md).

### DuckDB release construction and publication

The daily/manual refresh builds a new immutable DuckDB generation without
changing the active serving pointer. Snapshot queries are pinned explicitly to
that inactive candidate and cannot reuse active-generation snapshot responses.
The candidate is eligible for publication only after every route snapshot and
ancillary asset has passed validation.

Refresh identity is deterministic across the committed source manifest,
validated Arena/Records/tournament inputs, and the builder, route-contract,
serving, and snapshot code. Volatile fetch timestamps are excluded. A refresh
whose identity matches the active release is a no-op and does not advance the
last successful publication time. A matching inactive candidate left by a
failed publication is accepted only after its complete artifact inventory is
rechecked by byte count and SHA-256, then publication resumes without rebuilding
the database or snapshots.

Each publishable generation owns an authoritative artifact manifest. It lists
every mode-specific snapshot plus the two player autocomplete indexes, both
Home payloads and `home/defaults.js`, Arena latest/all-season/manifest assets,
and the complete default pack. Every entry records its object key, byte count,
SHA-256, kind, and data version where applicable. The release identity also
hashes the source generation, moving Arena/Records/tournament inputs, route
contract, public contract schema, and the builder/serving code. Publication is
therefore never inferred from the previous public pack's membership.

The VM publisher first uploads immutable generation-prefixed copies and creates
server-side rollback copies of all stable public objects. It primes the
candidate's interactive Cards relations while the previous generation is still
public, then advances the local serving and snapshot pointers, publishes stable
aliases, and writes the default pack last. A failed warm-up changes no pointer.
A later failure compensates both local pointers and every changed public object.
A durable transaction journal permits the worker to perform the same
compensation after a process kill or VM restart. The three-hour worker
deadline covers publication as well as building; a timed-out candidate cannot
remain active. The global refresh lock serializes releases and prevents the
independent Elo-leaderboard path from replacing snapshots or the pack during a
release.

Every successful generation retains a permanent inverse-publication descriptor:
the prior serving generation, prior snapshot pointer, stable-object inventory,
and server-side rollback locations. The transient crash journal is deleted only
after this descriptor is committed. `phase5_rollback_release.py` applies that
exact inverse; rollback is not inferred from whatever objects happen to exist
later.

The 4-GiB serving VM admits at most two analytical requests. Each DuckDB
connection uses one thread, an 800-MB memory ceiling, and a 90-second cancellation
deadline. Identical concurrent JSON requests share one execution. Refreshes
require at least 8 GiB free or 120% of the active database size, whichever is
larger. Local retention keeps the active generation, its rollback parent, every
generation referenced by a publication journal, and the three newest completed
generations; staging debris older than one day is removed only while no refresh
lock exists.

VM application code is distributed as one checksum-verified immutable archive.
`current-app` is an atomic symlink to the installed code release, and its pointer
retains the previous code release. A production bundle is fail-closed unless the
active data generation already has the exact rollback descriptor above. The
archive and descriptor are uploaded before the deployment pointer, so bootstrap
can never select a partial bundle.

Refresh status reaches `100%`, advances `last_completed_at`, and updates the
completed data version only after coordinated public publication succeeds.
Source/export, build, snapshot, validation, or publication failures retain the
previous successful timestamp and public release. The current executable fault
tests are in `tests/test_increment6_transactional_publication.py` in the backend
working copy.

### Operating budget and architecture assessment

The original operating-budget discussion used USD as a planning currency, but
the Cloud Billing account is denominated in EUR. The enforceable policy is
therefore a EUR 20 monthly warning and a EUR 40 monthly hard-stop notification;
these values are not exchange-rate-pegged to USD. The [data architecture assessment](analysis/dashboard-cost-architecture-review.md)
...
DuckDB is the selected local engine and every recognized public route has a
local query builder, so analytical requests do not intentionally fall back to
BigQuery. The route contract, frozen evidence, adversarial fixtures, independent
same-source reconciliation, and increment-specific gates are the parity
authority. The corrected migration is active behind the public DuckDB gateway;
the Cloud Function remains the maintenance and rollback boundary rather than a
public analytical fallback.
...
`phase5-filter-performance-20260925`, with data version
`phase5-source-sync-7efb47511584a75bd22e-20260924001145`. Its exact rollback
parent is `phase5-refresh-9086145ea79c7db43deb5752`. The public pack and
private generation use that same data version. The atomic private snapshot
pointer is `phase2_private_data/snapshots/current.json`; individual objects are
published before the default-pack pointer, which is the coherent-generation
boundary. The current release contains 102 public objects and a complete inverse
publication descriptor.

The active immutable application release is
`deploy-0a3902e28a47b1bb4744`. `/home/ascri/current-app` points to its directory
under `/home/ascri/releases/`, and both the API and refresh-worker systemd units
run through that symlink. The VM has a 40 GiB disk and retained about 14.2 GB
free after the current build and retention cleanup. Readiness reports the active
generation and `rollback_ready: true`.

The corrected cutover is certified by the Increment 8 evidence under
`analysis/`. The complete local suite passes 227 tests; JavaScript syntax checks
pass for both the working and publication trees; all 49 independently produced
route references pass; the full 13,456-row Card + Card output has no missing,
unexpected, or raw-statistic mismatch; all six retained component-CI batches,
three Players graph endpoints, and the Arena graph artifact pass. The explicit
coverage inventory contains 63 unique entries: 49 historical routes, Card +
Card, six component batches, three graph endpoints, and four ancillary artifact
checks. Snapshot validation expects and finds 98 snapshots; the complete release
contains 102 valid artifacts.

The rollback claim is based on a real two-way rehearsal, not only metadata. The
serving pointer and public pack were moved to the paired parent, verified, and
then restored to the current 102-artifact generation with matching data version.
Representative desktop and phone-width browser checks cover Cards, Arena, and
Players table/graph flows. In Players, selecting `trusting-seagull275` loads real
metrics without a synthetic Games row or undefined values, and the combined Elo
history renders successfully. These checks establish release evidence; they do
not imply permission to publish frontend files to GitHub.

Serving admits at most two analytical executions, uses one DuckDB thread and an
800 MB DuckDB memory limit per query, and interrupts public work after 90
seconds. Identical in-flight requests share one execution. Offline snapshot
construction pins the inactive candidate and uses a separate 600-second query
deadline. Refresh builds use two threads and a 2,000 MB DuckDB memory limit.
Default requests use immutable generation-local snapshots; restrictive filters
use the local SQL builders.

The production mixed-load gate ran real uncached non-default filters during the
complete current refresh. Actions completed in 32.952 seconds, Cards in 73.807
seconds, and Card + Card in 25.791 seconds. Peak gate-process RSS was
983,552,000 bytes and minimum available system memory was 710,447,104 bytes.
The refresh completed in 2 hours 24 minutes 40 seconds and passed atomic
publication, readiness, and public gateway smoke checks.
...
derivatives, imports Tournament/Arena/Records moving sources, generates and
validates the complete snapshot pack against the inactive candidate, and then
coordinates public objects, snapshot pointer, and serving activation as one
rollback-safe publication. A failed build or publication leaves the previous
successful release authoritative. It never publishes GitHub.
...
`refresh-private-duckdb-daily` at 00:00 UTC. It sends valid JSON and authenticates
with the dedicated `dashboard-refresh-scheduler` OIDC service account. The fixed
always-on VM performs the moving-source downloads, source import, local build,
snapshot generation, and atomic activation. Arena CSVs and Records spreadsheet
exports refresh on every run with last-known-good fallbacks.
`phase2_private_data/moving_sources/source-status.json` records `fresh` or
`last_known_good` for each source. An ongoing Arena season may exist in settings
before its Elite League roster is published; only ended seasons require a roster,
and missing/invalid end timestamps fail closed. Tournament metadata uses the
same fallback rule through its validated refresh-owned JSON cache and then the
active generation. The candidate manifest carries source states and fingerprints,
so fallback use is visible without exposing fetch internals. A refresh fails
before activation when neither fresh nor complete last-known-good required input
exists. The refresh status object exposes only sanitized state/progress and the
last successful completion. A Monitoring alert notifies `hlmichel.vo@gmail.com`
on refresh failures. The project quota is 0.1 TiB/day and expensive legacy
BigQuery schedules remain paused.

The local backend contract is currently `PUBLIC_CONTRACT_SCHEMA_VERSION = 4`
and `RESPONSE_CACHE_SCHEMA_VERSION = 8`. Immutable snapshots and cache entries
from earlier contract versions are not eligible for reuse. The route code may
be newer than the active VM generation; a new generation must be built,
validated, snapshotted, and atomically activated before schema-v4 behavior is
public. Never point the serving process at a partially rebuilt database.

Free metadata checks skip unchanged BigQuery source families. A changed Full
Sample family uses one bounded canonical export; Logs uses native extraction
when its own fingerprint changes. An unchanged repeat performs no BigQuery
query. All derivative and snapshot work then runs locally on the VM.
`cards_attributes.csv`, `merge_players.csv`, and the recursive `contracts/`
inventory participate in refresh identity and immutable deployment packaging.
Snapshot component-CI enrichment calculates each distinct displayed standalone
component once per request scope and reuses it across rows. Additive Synergy CIs
remain intentionally absent.

The serving choice is the always-on e2-medium VM. Billing safety rails are
...
charges or disable unrelated project services. BigQuery roles still required by
controlled source export and Elo maintenance remain in place; public BigQuery
reads and legacy analytical schedules remain disabled.

The live 2026-09-24 inventory confirms both project-scoped EUR budgets, the
shared Monitoring notification channel, the EUR 40 Pub/Sub topic, its active
OIDC-authenticated push subscription, and the private `budget-shutdown` Cloud
Run service. Revision `budget-shutdown-00006-ttz` receives 100% of traffic with
explicit `BUDGET_CURRENCY=EUR` and `BUDGET_HARD_LIMIT=40`; it stops only
`ark-nova-duckdb-test`, rejects an explicitly non-EUR notification, does not
restart the VM automatically, and is invokable only by its dedicated service
account.
...
| Predictors | paired focal-player/opponent observation | completed games; all sidebar predicates apply only to the focal player |
| Actions | player-game or action-log observation | Starting position, Upgrades, Upgrade order, and Upgrades by map are completed-only |
| Build / Conservation / Scoring / Icons / Workers | view-specific player-game or event | fixed display catalogs; per-subject valid-value denominators; hard/optional completion follows the view contract |
...
The sponsor catalogs are authoritative rather than inferred from observed data:
31 CP sponsors and nine Appeal sponsors define the valid rows and buckets. Every
bucket EV is the mean source `elo_delta` for the exact played-sponsor/endgame
value population. Endgame CP or Appeal is never substituted for EV. Ordinary
Student-t interval fields use that same EV population.
...
`MAINTENANCE_TOKEN` is stored in Secret Manager as `maintenance-token:latest`;
the backend runtime service account has accessor permission and the Cloud Run
revision uses a secret reference rather than a plain environment value. The
00:00 UTC Scheduler job uses the separate
`dashboard-refresh-scheduler` OIDC identity, valid JSON, and the matching
maintenance header. `REFRESH_PAGE_PASSWORD` remains a separate secret. Never
print a raw Scheduler job description: its headers can contain credential
values. Inspect only header names or compare the header to Secret Manager
in-memory and emit a boolean result.
...
Its 19 conditions are More endgame points, More endgame CP, More ingame CP,
More reefers, More small/medium/large animals, the four Round 1 conditions
(Upgrade, Project, Release, and 2+ association actions), the four First-to-5/8
CP conditions with and without exactly one university/partner-zoo bonus, and
the four starting-hand sponsor/project conditions. These labels are backed by
their actual log/endgame/opening-hand predicates; they are not aliases for a
generic comparison. General and Icons first pair both players by table, then
apply map, Elo, date, Starting-position, Arena, and Tournament predicates only
to the focal player. All Predictor views are completed-only.
...
Build/Hexes returns six compact buckets (`0`, `1-5`, `6-11`, `12-17`,
`18-23`, `24+`) plus a separate exact expanded set (`0` through `23`, then
`24+`) in `expanded_data`; expansion is local. Enclosures' Empty Petting Zoo
row means exactly one built Petting Zoo, zero Petting Zoo icons, and no played
Horse Whisperer. It is not a null/missing-enclosure bucket.
...
Players default General is snapshot-backed only when no player, player list, or
Last X scope is present. A normal selected-player request reads the physically
clustered `players_by_identity_narrow` relation and joins the selected moments
to the eight precomputed default cohort rows. It must not reuse the default
snapshot. A custom sidebar scope reads `players_metrics_narrow` and evaluates
the all, winner, expert, and master cohorts as separate compact aggregates;
those four populations must all change with the filters, so this path remains a
wider correctness-first scan. A new selected player at normal scope measured
1.68 seconds through the public gateway, and an exact repeat measured 0.15
seconds. Advanced Players sidebar scopes are not covered by that five-second
claim.
...
Cards also owns dataset-specific player facts and compact table/card moments for
the common linked-Elo scope. Players owns an identity-clustered copy and eight
default-cohort rows in addition to its general metric fact.
...
The Cloud Run gateway keeps one minimum instance. The private API primes both
dataset variants of the common Cards relation before opening its listening port;
publication performs the same prime against an inactive candidate before pointer
activation. A new linked-Elo Cards scope measured 3.16 seconds through the
public gateway and its exact repeat measured 0.20 seconds. A new normal-scope
selected player measured 1.68 seconds and its repeat 0.15 seconds. Card + Card is the
...
Leaderboard, manual Fastest, and Biggest Turns inputs are cached as one
refresh-owned moving-source set. A temporary Google Sheets delivery or
validation failure reuses the corresponding last-known-good validated file and
does not publish a partial Records population. Every leaderboard row must include a
...
the external source and cache metadata from `source-status.json`, so
last-known-good use is visible.
...
- Arena and Tournament switches appear on Home, Cards, Opening Hand, Endgames,
  ...
  off.
- Players sends `players_arena_seasons` only when at least one season is
  selected. No season restriction is represented by omitting the optional
  field, never by sending an empty array; the backend intentionally rejects an
  explicitly empty array so malformed restrictive requests cannot silently
  broaden into the unfiltered population.
